...
class DQN:
    def __init__(self, num_input=None, num_output=None, list_hidden=None, path=None, path_append=None):
        ...
        if path == None: # create 
            if path_append == None: # create -> new
                ...
                target_model = self.create_model(num_input, num_output, list_hidden)
            else: # create -> append
                model = self.append_model(num_input, num_output, path_append)
                target_model = self.append_model(num_input, num_output, path_append)
        else: # load old model
            model_num_input, model_num_output, model = self.load_old_model(path)
    ...
    def append_model(self, num_append_input, num_append_output, path_append):
        ...
        for indx,layer in enumerate(old_model.layers):
            ...
            if indx == 0:          
                ...
                new_model.add( Dense(size_node, input_dim=new_input, activation="relu") )
                # print("before ")
                ...
                for i in range(num_append_input): # create zero weight
                    ...
                    new_weight[0] = np.append(new_weight[0], np.zeros(size_node).reshape(1,size_node) , axis=0) 
                ...
                for i in range(num_append_output):
                    new_weight[0] = np.insert(new_weight[0], size_node, 0, axis=1)
        ...
        new_model.compile(loss="mean_squared_error", optimizer=Adam(lr=self.learning_rate))
        return new_model
    ...
    def create_model(self, num_input, num_output, list_hidden):
        ...
        model = Sequential()
        model.add(Dense(list_hidden[0], input_dim= num_input, activation="relu"))
