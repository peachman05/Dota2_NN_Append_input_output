...
def runControl(dqn_agent, data, pathSaveFile):
  ...
  if data['method'] == GET_DQN_DETAIL:
    ...
    print(dqn_agent)
    weight_layer = dqn_agent.model.layers[ layer-1 ].get_weights() # use lua indexing (start with 1 )
    weight_dict = {}
    weight_dict['weight'] = weight_layer[0][row-1].tolist()
    return_thing =  jsonify(weight_dict)

  elif data['method'] == GET_BIAS:    
    weight_layer = dqn_agent.model.layers[ data['layer']-1 ].get_weights() # use lua indexing (start with 1 )
    weight_dict = {}
